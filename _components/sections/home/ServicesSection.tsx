...
const ServicesSection = ({ data }: ServicesSectionProps) => {
  ...
  return (
    <Bounded ref={servicesRef} id="services" className="py-8">
      <div className="grid grid-cols-1 gap-5 py-8 md:grid-cols-2  xl:grid-cols-4">
        {data?.services?.map((service:Service) => (
          <Link href={service?.href || "#"} key={service?.id}>
            <div className="service-card relative h-60 cursor-pointer overflow-hidden rounded-tl-4xl rounded-br-4xl shadow-lg group">
              <Image
                src={getStrapiMedia(service?.image?.url) ?? "/assets/jpg/profile-placeholder.svg"} 
